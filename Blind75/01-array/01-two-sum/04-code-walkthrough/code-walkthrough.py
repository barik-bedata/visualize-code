...
class Walkthrough(Scene):
    ...
    return new int[] {};
        ...
        # Line 1: Map<Integer, Integer> map = new HashMap<>();
        highlight_line(1)
        self.wait(0.5)
        ...
        for i in range(len(nums)):
            ...
            if is_found:
                final_status_txt = Text(f"Checking Map: {comp} ✓", font=typo.font_ui(), font_size=16, color=GREEN)
                ...
                final_status_txt = Text(f"Checking Map: {comp} ✗", font=typo.font_ui(), font_size=16, color=RED)
                final_status_txt.move_to(search_status_txt.get_center())
                self.play(ReplacementTransform(search_status_txt, final_status_txt), run_time=0.3)
