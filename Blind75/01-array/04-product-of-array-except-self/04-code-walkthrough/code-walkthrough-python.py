...
class WalkthroughPython(Scene):
    ...
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        for i in range(len(nums_array)-1, -1, -1):
            # Line 11: for i in range(n - 1, -1, -1):
            self.play(highlight_line(11), run_time=0.3)
            self.play(i_ptr.animate.next_to(ans_cells[i][0], DOWN, buff=0.4), run_time=0.3)
            
            mid_y = (cells[0][0].get_y() + ans_cells[0][0].get_y()) / 2 + 0.3

            # Line 12: ans[i] = ans[i] * suffixProd
            ...
            # Line 13: suffixProd = suffixProd * nums[i]
