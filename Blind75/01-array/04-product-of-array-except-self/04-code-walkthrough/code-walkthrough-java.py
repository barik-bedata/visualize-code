...
class WalkthroughJava(Scene):
    def construct(self):
        ...
        tracker.screen_code_walkthrough("Java Code Walkthrough")
    ...
    public int[] productExceptSelf(int[] nums) {
        ...
        # Line 12: int postfix = 1;
        self.play(highlight_line(12), run_time=0.5)
        postfix = 1
        ...
        for i in range(len(nums_array)-1, -1, -1):
            # Line 13: for (int i = n - 1; i >= 0; i--)
            self.play(highlight_line(13), run_time=0.3)
            self.play(i_ptr.animate.next_to(ans_cells[i][0], DOWN, buff=0.5), run_time=0.3)
            
            # Line 14: ans[i] *= postfix;
            ...
            # Line 15: postfix *= nums[i];
            self.play(highlight_line(15), run_time=0.3)
        ...
        # Line 18: return ans;
        self.play(highlight_line(18))
        self.play(FadeOut(prefix_text), FadeOut(postfix_text), FadeOut(i_ptr), run_time=0.5)
