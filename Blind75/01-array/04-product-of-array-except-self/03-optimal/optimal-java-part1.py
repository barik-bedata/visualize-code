...
class OptimalJava(Scene):
    def construct(self):
        ...
        for i in range(1, len(nums_array)):
            ...
            # Build equation prefixProd = el1 x el2 ...
            mid_y = (cells[0][0].get_y() + ans_cells[0][0].get_y()) / 2 + 0.3
            
            eq_items = [Text("prefixProd =", font=typo.font_code(), font_size=20, color=WHITE)]
            for j in range(i):
            ...
            # Highlight the current answer cell
            ans_cells[i][0].set_z_index(1)
            self.play(
                FadeIn(calc_group[0]),
                ans_cells[i][0].animate.set_stroke(YELLOW, width=4),
                run_time=0.3
            )
            ...
            fade_outs = [FadeOut(c) for c in copies] + [FadeOut(calc_group)] + [FadeOut(eq_res[0])] + [FadeOut(ans_cells[i][1])]
            
            self.play(
                ReplacementTransform(eq_res[1], ans_val),
